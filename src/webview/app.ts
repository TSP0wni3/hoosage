    ...
    !calls.length
      ...
      : sorted
          ...
          .map(([key, list]) => {
            ...
            return `<div class="session"><button class="session-toggle" data-session="${h(key)}" aria-expanded="${expandedSession === key}"><span class="session-icon">${icon("activity")}</span><span class="session-title"><strong>${h(p?.name ?? "Unknown project")}</strong><small>${new Date(latest).toLocaleString("en", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}${first.sessionId ? "" : " · Unlinked calls"}</small></span><span class="session-count">${count(t.calls, "call")}</span><strong class="session-usage">${h(costLabel(costs(list)))}<small>${compact(t.tokens)} tokens</small></strong>${icon("chevron")}</button>${
              expandedSession === key
