...
4. Click **Enable Chat tracking once**, reload the window when prompted, and use Copilot Chat. The setting applies to all trusted projects in this VS Code profile. Other already-open windows need one reload after initial setup; newly opened projects register automatically. A folder listed only under **File → Open Recent** registers when you open it with hoosage installed; its earlier Chat usage cannot be reconstructed.
...
A VS Code project appears when its workspace is open in a trusted window with hoosage running, when hoosage has registered it in an earlier window, or when a completed local Copilot CLI or JetBrains session identifies it by working directory. Hoosage does not scan **File → Open Recent**: VS Code's public extension API lists the folders in the current window, not the editor's recent-workspace history. A folder found only in Open Recent, never registered by hoosage and without a completed local session, therefore has no project card. Opening it once registers it for future Chat tracking without another Enable click. The first registration does not imply any past usage; a dash in the project table means no saved measurement in the selected period, not a measured zero.
